...
class S2VTModel(nn.Module):
    def __init__(self, vocab_size, max_len, dim_hidden, dim_word, dim_vid=4096, sos_id=1,
                 n_layers=1, rnn_cell='lstm', rnn_dropout_p=0.2,):
        '''
        Args: vocab_size(int): size of vocab
              max_len(int): max length of caption
              dim_hidden(int): dimension of hidden state
              dim_word(int): dimension of word embedding
        	  dim_vid(int): dimension of video features (4096)
              sos_id(int): index of <'sos'> in vocab
              n_layers(int): number of layers in rnn
              rnn_cell(str):  name of rnn ('lstm', 'gru')
        '''
